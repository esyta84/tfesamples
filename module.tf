 ...
 * # vSphere VM Module
 * 
 * A Terraform module for deploying VMs in vSphere with tenant-specific clusters.
 * Supports multiple operating systems and automatically selects appropriate resources.
 */
...
variable "tenant_cluster" {
  description = "vSphere cluster name representing the tenant"
  type        = string
}
...
locals {
  # Extract tenant name from cluster for naming
  tenant_name = split("-", var.tenant_cluster)[0]
  ...
  # Set folder based on tenant cluster if not explicitly specified
  vm_folder = var.folder != "" ? var.folder : "/Tenants/${local.tenant_name}"
  
  # Set resource pool based on user input or default to cluster root
  resource_pool_path = var.resource_pool != "" ? "${var.tenant_cluster}/${var.resource_pool}" : var.tenant_cluster
}
...
# Find datastores matching the regex pattern or use query to find one with most free space
...
# Find network in the appropriate cluster
...
resource "vsphere_virtual_machine" "vm" {
    ...
    customize {
      ...
      dynamic "windows_options" {
        ...
        content {
          ...
          organization_name = local.tenant_name
  ...
  annotation = var.annotation != "" ? var.annotation : "Managed by Terraform. Tenant Cluster: ${var.tenant_cluster}, OS: ${var.operating_system}"
...
output "tenant_cluster" {
  description = "The tenant cluster used for VM deployment"
  value       = var.tenant_cluster
}
